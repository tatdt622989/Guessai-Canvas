...
function RankingList() {
  ...
  const items = useMemo(() => {
    const arr = rankingList.map((item, i) => {
      return item.score ? (
        <div
          className="ranking-item d-flex align-items-center position-relative"
          key={i}
        >
          <div className="ranking-no d-flex justify-content-center align-items-center me-3">
            <Hexagon />
            <span className="number">{i + 1}</span>
          </div>
          <div className="ranking-item-photo d-block">
            {item.photo ? (
              <img
                src={`${API_URL}/guessai_canvas/user_photo/${item.photo}/`}
                alt=""
  ...
  return <div className="ranking-list">{items}</div>;
}
...
function Ranking(props: Prop) {
  ...
  return (
    <div className="ranking overflow-hidden" style={{ maxHeight: height }}>
      <div
        className="ranking-container overflow-auto position-relative"
